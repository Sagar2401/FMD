...
function minPlanesUsed(arr) {

    if (arr[0] === 0) return -1;

    let maxAttempt = arr[0];
    let steps = arr[0];
    let planesUse = 1;
    ...
    for (let i = 1; i < arr.length; i++) {
        ...
        if (i === arr.length - 1) return planesUse;

        maxAttempt = Math.max(maxAttempt, i + arr[i]);

        steps--;

        if (steps === 0 && i < arr.length - 1) {

            if (maxAttempt <= i) return -1;
...
console.log(minPlanesUsed([1, 6, 3, 4, 5, 0, 0, 0, 6]));
